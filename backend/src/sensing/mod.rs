...
mod tests {
    ...
    use super::sensor::{MockSensor, DataPoint};
    ...
    fn read_sensor_data() {
        ...
        factory_mock.expect_create().returning(move |_| {
            ...
            sensor_mock.expect_read_data().return_const(Some(sensor_data));
            Box::new(sensor_mock)
        });
        ...
        assert!( approx_eq!(f32, sensor_data.temperature, read_data.unwrap().temperature, ulps = 2) );
        assert!( approx_eq!(f32, sensor_data.humidity, read_data.unwrap().humidity, ulps = 2) );
    }
}